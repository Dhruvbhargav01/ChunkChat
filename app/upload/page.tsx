  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage('Upload success! User is allowed to upload file after 1 min.')

        // ✅ Start 1-minute cooldown
        setCanUpload(false)
        setTimeout(() => setCanUpload(true), 60 * 1000)
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#3b0b8f] via-[#020617] to-[#3b0b8f] text-slate-100">
      <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-gradient-to-br from-[#050816] via-black to-[#050816] shadow-[0_0_45px_rgba(139,92,246,0.45)] p-8 space-y-5">
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center bg-gradient-to-r from-[#8b5cf6] via-[#a855f7] to-[#4c1d95] text-white px-5 py-2.5 rounded-full text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_0_28px_rgba(168,85,247,0.7)] hover:shadow-[0_0_40px_rgba(236,72,153,0.9)] transition-shadow"
        >
          {uploading ? 'Uploading…' : 'Upload and process'}
        </button>

        {/* ✅ Chat button matching current theme */}
        <button
          onClick={handleGoToChat}
          className="w-full inline-flex items-center justify-center mt-2 bg-gradient-to-r from-[#6727fe] via-[#733ba8] to-[#4c1d95] text-white px-5 py-2.5 rounded-full text-sm font-medium shadow-[0_0_28px_rgba(168,85,247,0.7)] hover:shadow-[0_0_40px_rgba(236,72,153,0.9)] transition-shadow"
        >
          Go to Chat
        </button>
